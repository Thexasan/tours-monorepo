...
>
> 🔥 **Перед тем как делать что-либо — обязательно прочитай `STATUS.md`** (живое состояние «что сделано, что нет, что отложено»). А если задача про email — прочитай ещё `EMAIL_SETUP.md`.
>
> Полное ТЗ — `Traveling tour system.pdf` в корне. План работ и прогресс — `PLAN.md`. Деплой — `DEPLOY.md`.
