...
export class EmailService {
  ...
  /**
   * Базовая отправка через Resend (https://resend.com/docs/api-reference/emails/send-email).
   * Если ключа нет — логируем письмо в консоль (для разработки).
   */
  ...
  async sendBookingReceived(to: string, contactName: string, tourTitle: string, totalPrice: number): Promise<void> {
    await this.send({
      to,
