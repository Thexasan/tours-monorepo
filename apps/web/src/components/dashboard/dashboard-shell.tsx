...
export function DashboardShell({ children }: { children: React.ReactNode }) {
  ...
  return (
    <>
      {/* ── Mobile top header ───────────────────────────────────── */}
      <header
        className="lg:hidden fixed top-0 inset-x-0 z-50 flex items-center justify-between px-4 h-14"
        style={{
      ...
      >
        ...
        <div className="flex items-center gap-2.5">
          {user?.avatarUrl ? (
            ...
            <div className="h-8 w-8 rounded-xl bg-linear-to-br from-emerald-600 to-teal-600 text-white grid place-items-center text-xs font-black shrink-0">
              {initials || "T"}
            </div>
          )}
          <div className="min-w-0">
            ...
            <p className="text-[10px] text-slate-400 mt-0.5">{t('clientTitle')}</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[290px_1fr] gap-8 pt-[72px] pb-8 lg:py-8">
        ...
        {/* ── Main content ─────────────────────────────────────────── */}
        <main className="min-w-0 overflow-x-hidden pb-32 lg:pb-0">{children}</main>
