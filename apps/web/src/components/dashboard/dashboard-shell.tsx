    ...
    <div className="grid grid-cols-1 lg:grid-cols-[290px_1fr] gap-8 py-8">
      ...
      <aside className="hidden lg:block lg:sticky lg:top-6 lg:self-start h-fit">
        <div className="tv-surface-elevated overflow-hidden">
          ...
          <div className="relative p-5 pb-4">
            ...
            <div className="relative mt-4 flex items-center justify-between">
              <span className={`tv-chip ring-1 ${rm.cls}`} style={{ background: "rgba(255,255,255,0.95)" }}>
                <span className="h-1.5 w-1.5 rounded-full bg-current" />
                {rm.label}
              </span>
              <Link href={`/${locale}/tours`} className="text-xs font-medium text-white/95 hover:text-white inline-flex items-center gap-1">
          ...
          <nav className="px-3 pb-3 pt-1 flex flex-col gap-0.5">
            <p className="px-3 py-2 text-[11px] font-semibold uppercase tracking-[0.08em] text-slate-400">Личный кабинет</p>
            {nav.map((it) => {
        ...
        <div className="mt-5 relative overflow-hidden rounded-2xl p-5 text-white tv-hero-sunset">
          ...
          <p className="mt-2 font-semibold text-lg leading-snug">Приведи 5 друзей — получи тур за наш счёт ✨</p>
          <Link href={`/${locale}/dashboard/referrals`} className="mt-3 inline-flex items-center gap-1 text-sm font-semibold underline-offset-2 hover:underline">
            Узнать подробнее <ChevronRight className="h-4 w-4" />
          </Link>
        </div>
      </aside>
