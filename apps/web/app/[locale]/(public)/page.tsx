...
const TESTIMONIAL_TONES: Record<string, { bg: string; text: string }> = {
  teal:  { bg: "from-teal-500 to-teal-700",  text: "text-teal-700"  },
  rose:  { bg: "from-teal-500 to-teal-700",  text: "text-teal-700"  },
  amber: { bg: "from-teal-500 to-teal-700",  text: "text-teal-700"  },
  sky:   { bg: "from-teal-500 to-teal-700",  text: "text-teal-700"  },
};
...
export default async function HomePage() {
  ...
  return (
    <div className="flex flex-col w-full bg-[#f8fafc] font-sans selection:bg-teal-500/30">
      ...
      <section className="relative -mt-16 h-[100vh] min-h-[700px] overflow-hidden">
        ...
        {[
          ...
          <div
            ...
            style={{
              animationDelay: i === 0 ? "-1.44s" : `${i * 6}s`,
              opacity: i === 0 ? 1 : 0,
              zIndex: 0
            }}
          >
        ...
        <div className="relative z-10 h-full flex flex-col justify-center pt-24 pb-16">
          <div className="mx-auto w-full max-w-7xl px-4 md:px-6 lg:px-8 flex flex-col lg:flex-row items-center justify-between gap-12">
            ...
            <div className="flex-1 text-center lg:text-left mt-8 lg:mt-0">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 backdrop-blur-md border border-white/20 mb-6 shadow-2xl transition-transform hover:scale-105">
                ...
                <span className="text-white/90 text-sm font-medium tracking-wide">Путешествуй без границ</span>
              </div>
              
              <h1 className="font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-white to-white/70 leading-[1.1] tracking-tight drop-shadow-2xl mb-6 text-5xl sm:text-6xl lg:text-7xl xl:text-8xl" style={{ filter: 'drop-shadow(0px 4px 20px rgba(0,0,0,0.5))' }}>
                Открой мир <br />
              ...
              <p className="text-lg md:text-xl text-white/80 max-w-2xl mx-auto lg:mx-0 leading-relaxed mb-8 font-light">
                Роскошные туры по честным ценам. Бронируйте премиум-отдых или путешествуйте абсолютно бесплатно благодаря нашей уникальной реферальной программе.
              </p>
              ...
              <div className="flex flex-wrap items-center justify-center lg:justify-start gap-4 mb-10">
                <div className="flex items-center gap-2">
                  ...
                  <div className="ml-2">
                    <p className="text-white font-bold text-sm">12,000+</p>
                    <p className="text-white/60 text-xs">Довольных туристов</p>
                  </div>
                </div>
                <div className="w-px h-8 bg-white/20 hidden sm:block mx-2" />
                <div className="flex items-center gap-1.5">
                  {[1,2,3,4,5].map(i => <Star key={i} className="w-5 h-5 fill-amber-400 text-amber-400" />)}
                  <span className="text-white font-bold text-sm ml-1">4.9/5 рейтинг</span>
                </div>
              </div>
            </div>

            {/* Glassmorphic Search Widget Right Side */}
              ...
              <div className="bg-white/10 backdrop-blur-2xl border border-white/20 p-1 rounded-3xl shadow-[0_30px_60px_rgba(0,0,0,0.4)] relative overflow-hidden">
                ...
                <div className="bg-slate-900/40 backdrop-blur-xl p-6 rounded-[1.35rem] relative z-10 border border-white/10">
                  ...
                  <div className="mt-6 pt-5 border-t border-white/10">
                    <p className="text-white/50 text-xs uppercase tracking-wider mb-3 font-semibold">Популярные запросы</p>
                    <div className="flex flex-wrap gap-2">
                      {["Бали", "Мальдивы", "Дубай", "Таиланд"].map((dest) => (
                        <Link
                          key={dest}
      ...
      {/* ───── TRUST FEATURES — Glow Cards ───── */}
      <section className="relative z-20 -mt-8 px-4 md:px-6 lg:px-8 max-w-7xl mx-auto w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
          {[
            { icon: Shield, title: "Безопасность 100%", text: "Защищённые платежи и гарантия возврата", tone: "teal" },
            { icon: Headphones, title: "Поддержка 24/7", text: "Ваш личный менеджер всегда на связи", tone: "teal" },
            { icon: Award, title: "Лучшие цены", text: "Никаких скрытых комиссий и наценок", tone: "teal" },
            { icon: Sparkles, title: "Бесплатные туры", text: "Получайте туры за приглашение друзей", tone: "teal" }
          ].map((feature, idx) => (
      ...
      {/* ───── DESTINATIONS — Premium Bento Grid ───── */}
      <section className="py-24 md:py-32">
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8">
          <SectionHeader 
            eyebrow="Исследуйте мир" 
            title="Топовые направления" 
            description="Культовые курорты, экзотические острова и лучшие пляжи мира ждут вас."
            action={
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-4 grid-rows-[auto] gap-4 md:gap-6">
            {DESTINATIONS.map((dest, idx) => {
              // Create a striking bento layout
              const isLarge = idx === 0;
                ...
                >
                  ...
                  <div className="absolute bottom-0 left-0 p-6 sm:p-8 w-full transform group-hover:-translate-y-2 transition-transform duration-500">
                    <div className="flex items-center gap-2 mb-2 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-100">
                      ...
                      <span className="text-teal-400 text-xs font-bold uppercase tracking-wider">{dest.country}</span>
                    </div>
                    <h3 className={`${isLarge ? 'text-4xl md:text-5xl' : 'text-2xl md:text-3xl'} font-black text-white tracking-tight mb-2`}>
                      {dest.name}
                    </h3>
                    <div className="flex items-center justify-between text-white/80 text-sm font-medium">
                      <span className="flex items-center gap-1.5"><MapPin className="w-4 h-4" /> {dest.tours} туров доступно</span>
              ...
              {hotTours.slice(0, 4).map((tour, index) => {
                ...
                return (
                  <div key={tour.id} className={`group relative rounded-[2rem] overflow-hidden transition-all duration-700 hover:-translate-y-2 hover:shadow-[0_30px_60px_rgba(243,24,70,0.15)] cursor-pointer ${spanClasses}`}>
                    ...
                    {/* Gradient Overlays for readability - much deeper and darker at bottom */}
                    ...
                    {/* Floating HOT badge - minimal and sleek */}
                    ...
                    {/* Content positioned directly on the dark gradient */}
                    <div className="absolute inset-x-0 bottom-0 p-6 md:p-8 flex flex-col justify-end">
                      <div className="transform translate-y-3 group-hover:translate-y-0 transition-transform duration-500">
                        ...
                        <h3 className={`font-black text-white leading-[1.15] mb-6 drop-shadow-lg ${index === 0 ? "text-3xl md:text-5xl" : "text-2xl"}`}>
                          {tourTitle}
                        </h3>
                        ...
                        <div className="flex items-end justify-between pt-5 border-t border-white/10">
                          <div>
                            <p className="text-[10px] text-white/50 uppercase tracking-[0.15em] font-bold mb-1">Стоимость</p>
                            <p className="text-3xl font-black text-white flex items-baseline gap-1">
                              ${tour.priceUsd} <span className="text-sm font-medium text-white/50">/чел</span>
                            </p>
                          </div>
      ...
      {/* ───── HOW IT WORKS — Ultra Premium Asymmetric Layout ───── */}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-12 items-start">
            ...
            <div className="lg:col-span-5 lg:sticky lg:top-32 pt-4">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-50 border border-slate-200 text-slate-600 text-xs font-bold uppercase tracking-[0.15em] mb-8 shadow-sm">
                <Sparkles className="w-3.5 h-3.5 text-teal-500" /> Программа лояльности
              </div>
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
                Путешествуй <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-slate-400 to-slate-600">за наш счет</span>
              </h2>
              <p className="text-slate-500 text-lg md:text-xl leading-relaxed mb-10 max-w-md font-medium">
                Делитесь эмоциями и зарабатывайте баллы на реальные поездки. Никаких скрытых условий — всё кристально прозрачно.
              </p>
              ...
              <Link href={`/${locale}/register`} className="inline-flex items-center gap-3 px-8 py-4 bg-slate-900 text-white font-bold text-lg rounded-2xl hover:bg-slate-800 transition-all shadow-xl shadow-slate-900/20 hover:-translate-y-1">
                Получить ссылку <ArrowRight className="w-5 h-5" />
              </Link>
            </div>

            {/* Right Column: Sleek Typography Cards */}
              ...
              {[
                {
                  ...
                  desc: "Создайте личный аккаунт и отправьте вашу уникальную реферальную ссылку друзьям.",
                  accent: "bg-teal-500"
                },
                {
                  step: "02",
                  icon: ShoppingBag,
                  title: "Друзья бронируют",
                  desc: "За каждую оплаченную заявку от друга вы получаете бонусы прямо в личный кабинет.",
                  accent: "bg-teal-500"
                },
                {
                  step: "03",
                  icon: Plane,
                  title: "Летите на море",
                  desc: "Накопите баллы и выберите любой тур из нашего каталога абсолютно бесплатно.",
                  accent: "bg-teal-500"
                }
              ].map((item, idx) => (
                <div key={idx} className="relative bg-white rounded-[2.5rem] p-8 md:p-12 border border-slate-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] overflow-hidden group hover:-translate-y-2 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.08)] transition-all duration-500">
                  ...
                  <div className="relative z-10 flex flex-col sm:flex-row sm:items-center gap-6 md:gap-10">
                    ...
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-3xl bg-slate-50 border border-slate-100 flex items-center justify-center shrink-0 shadow-sm relative overflow-hidden group-hover:bg-white transition-colors duration-500">
                      <div className={`absolute top-0 left-0 w-full h-1 ${item.accent} transform origin-left scale-x-50 group-hover:scale-x-100 transition-transform duration-500`} />
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-3">
                        <div className={`w-2 h-2 rounded-full ${item.accent} animate-pulse shadow-sm`} />
      ...
      {/* ───── TESTIMONIALS — Ultra Premium Staggered Masonry ───── */}
      <section className="py-24 md:py-32 bg-slate-50 relative overflow-hidden border-b border-slate-100">
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-6 lg:px-8 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-16 lg:gap-12 items-start">
            ...
            <div className="lg:col-span-5 lg:sticky lg:top-32 pt-4">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-slate-50 border border-slate-200 text-slate-600 text-xs font-bold uppercase tracking-[0.15em] mb-8 shadow-sm">
                <Star className="w-3.5 h-3.5 fill-amber-400 text-amber-400" /> Отзывы клиентов
              </div>
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 tracking-tight leading-[1.1] mb-6">
                Эмоции, <br/>
                <span className="text-transparent bg-clip-text bg-gradient-to-r from-teal-500 to-teal-400">которые бесценны</span>
              </h2>
              <p className="text-slate-500 text-lg md:text-xl leading-relaxed mb-10 max-w-md font-medium">
                Послушайте тех, кто уже открыл для себя новый уровень путешествий. Более 12,000 довольных туристов доверяют нам свой отдых.
              </p>
              ...
              <Link href={`/${locale}/tours`} className="inline-flex items-center gap-3 px-8 py-4 bg-white border border-slate-200 text-slate-900 font-bold text-lg rounded-2xl hover:bg-slate-50 transition-all shadow-sm hover:shadow-md hover:-translate-y-1">
                Все направления <ArrowRight className="w-5 h-5" />
              </Link>
            </div>

            {/* Right Column: Staggered Masonry Reviews */}
                  ...
                  {STATIC_TESTIMONIALS.filter((_, i) => i % 2 === 0).map((tm, idx) => {
                    ...
                    return (
                      <div key={idx} className="relative bg-white rounded-[2.5rem] p-8 md:p-10 border border-slate-100 shadow-[0_20px_60px_-15px_rgba(0,0,0,0.05)] overflow-hidden group hover:-translate-y-2 hover:shadow-[0_30px_60px_-15px_rgba(0,0,0,0.08)] transition-all duration-500">
                        ...
                        <div className="absolute -top-10 -right-6 text-[180px] font-serif font-black text-slate-50 leading-none select-none pointer-events-none group-hover:-rotate-12 transition-transform duration-700 origin-center">
                          "
                        </div>
                        ...
                        <div className="relative z-10 flex flex-col h-full">
                          ...
                          <p className="text-slate-700 text-lg font-medium leading-relaxed mb-10">
                            "{tm.text}"
                          </p>

                          {/* User Info directly integrated, no colored block */}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
          ...
          <div className="bg-white rounded-[2.5rem] p-8 md:p-12 relative shadow-xl shadow-slate-200/50 border border-slate-100 overflow-hidden group">
            ...
            <h2 className="text-3xl md:text-5xl font-black text-slate-900 mb-6 leading-[1.1] tracking-tight">
              Индивидуальный <br/>подбор тура
            </h2>
            <p className="text-slate-500 text-lg mb-10 max-w-sm leading-relaxed">
              Без долгих поисков. Просто напишите, куда хотите отправиться, и мы организуем всё за 15 минут.
            </p>
            ...
            <div className="space-y-4 mb-10 relative z-10 w-full max-w-sm">
              <div className="flex justify-end transform hover:-translate-y-1 transition-transform">
                <div className="bg-slate-900 text-white px-5 py-3.5 rounded-2xl rounded-tr-sm text-sm font-medium shadow-md">
                  Хотим на Бали вдвоём, бюджет $2000 🌴
                </div>
              </div>
              <div className="flex items-end gap-3 transform hover:-translate-y-1 transition-transform">
                ...
                <div className="bg-white border border-slate-100 text-slate-800 px-5 py-3.5 rounded-2xl rounded-bl-sm text-sm font-medium shadow-lg shadow-slate-100">
                  Отличный выбор! Нашла 3 шикарных виллы. Отправляю варианты ✈️
                </div>
              </div>
            </div>
          ...
          <div className="rounded-[2.5rem] relative shadow-xl overflow-hidden group min-h-[500px]">
            ...
            <div className="absolute inset-0 p-8 md:p-12 flex flex-col justify-end">
              <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/20 backdrop-blur-md text-white text-xs font-bold mb-6 w-max border border-white/30 shadow-lg">
                <Sparkles className="w-3.5 h-3.5 text-teal-300" /> Клуб путешественников
              </div>

              <h2 className="text-3xl md:text-5xl font-black text-white mb-4 leading-[1.1] tracking-tight drop-shadow-lg">
                Путешествуйте <br/><span className="text-teal-300">бесплатно</span>
              </h2>
              <p className="text-white/90 text-lg mb-8 max-w-md leading-relaxed font-medium drop-shadow">
                Приглашайте друзей через персональную ссылку и копите бонусы за их бронирования. Накопите нужное количество бонусов и отправляйтесь на море за наш счёт!
              </p>
              ...
              <Link href={`/${locale}/register`} className="flex items-center justify-center gap-2 px-8 py-4 rounded-2xl bg-white text-slate-900 font-bold text-lg hover:bg-slate-50 transition-all hover:-translate-y-1 w-max shadow-xl shadow-black/20">
                Присоединиться к клубу <ArrowRight className="w-5 h-5" />
              </Link>
            </div>
          </div>
...
function SectionHeader({ eyebrow, title, description, action, center }: any) {
  return (
